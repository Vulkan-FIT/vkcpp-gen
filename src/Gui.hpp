...
class GUI {
    ...
    void createWindow() {
        ...
        int w = mode->width * 3 / 4;
        int h = mode->height * 3 / 4;
        glfwWindow = glfwCreateWindow(w, h, "Vulkan C++20 generator", NULL, NULL);
        if (!glfwWindow) {
        ...
        [](GLFWwindow* window, auto... args) {\
            auto pointer = static_cast<GUI::Window*>(glfwGetWindowUserPointer(window));\
    ...
    void initImgui() {        
        ...
        const auto immediate_submit =
            [&](std::function<void(VkCommandBuffer cmd)> &&function) {
                ...
                if (vkQueueSubmit(graphicsQueue, 1, &submit, fence) !=
                    VK_SUCCESS) {
    ...
    void updateImgui() {
        ...
        if (gen.isLoaded()) {
                ...
                for (auto &p : platforms) {
                    ...
                    if (ImGui::TreeNodeEx(p.first.c_str(),
                        ...
                        for (auto &e : extensions) {
                            if (e.second.platform && e.second.platform->first == p.first) {
            ...
            if (ImGui::InputText("Path", &regInputText)) {                
                regButtonDisabled =
                    !std::filesystem::is_regular_file(regInputText);
            }
